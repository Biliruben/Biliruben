...
public final class SplitXml {	
	...
	private static void setup() {
		...
		SplitterHandler handler = new SplitterHandler(outDir);
/*
		if (runner != null) {
		...
		*/
		...
		if (_opts.getStr(OPT_UNIQUE_ATTR) != null) {
			handler.setUniqueAttr(_opts.getStr(OPT_UNIQUE_ATTR));
		} else {
			handler.setUniqueAttr(DEFAULT_UNIQUE_ATTR);
		}

		handler.setUseNumbers(Boolean.parseBoolean(_opts.getStr(OPT_USE_NUMBER)));
